...
function createStream(text) {
  ...
  return new ReadableStream({
    ...
    start(controller) {
      controller.enqueue(
        Buffer.from("data:" + text + chunks[0])
      );
...
function makeString(length) {
  ...
  //add spaces
  const stringSplit = result.match(/.{1,4}/g);
...
// function makeString(length) {
  ...
  t.match(await parseAndOutputStream(encString), text.body);
